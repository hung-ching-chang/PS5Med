...
power.sim <- function(
  ...
  for(rep.time in 1:replication){
    ...
    PS5.result <- PS5.multisplit(M = mediator,
                                 X = exposure.mat,
                                 Y = diz,
                                 C = conf.mat,
                                 n.draw = 10000,
                                 ...
                                 cores = 7)
    selected.mediators <- sapply(row.names(PS5.result$mediation.contri),
                                 function(x) which(x == paste0("M", c(1:p))))
    ...
    selected.mediators <- sapply(row.names(HIMA.result),
                                 function(x) which(x == paste0("M", c(1:p))))
    ...
    global.me[rep.time,] <- c(PS5.result$global.me,
                              HP2016.result$global.me,
                              sum(HIMA.result[,4]),
                              hilma.result$beta_hat)
  }
  power <- apply(All.PVAL < 0.05, 2, function(x) mean(x, na.rm=T))
  ME.bias <- apply(global.me, 2,
                   function(x) mean(abs(x-true.me)/true.me, na.rm=T))
...
for(effect.a in effect.a.setting){
  ...
  power.result[[count]] <- power.sim(N = N.setting,
                                     ...
                                     replication = 100,
                                     multi.split = 100)
  save(power.result, file = paste0("power_N", N.setting,
                                   "p", p.setting,
                                   "_cor", cov.setting,
                                   "ts", num.ts.setting, ".RData"))
}
